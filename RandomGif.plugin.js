        ...
        return class RandomGif extends Plugin {
          ...
          addCommand() {
            Patcher.after(SlashCommandStore, "Kh", (_, args, res) => {
              ...
              res.push({
                ...
                displayDescription: "Sends a Random gif from Tenor.",
                description: "Sends a Random gif from Tenor out of the first 500 for a provided search.",
          ...
          async getGif(send) {
            const response = await fetch(
              "https://g.tenor.com/v1/random?q="+send+"&key=ZVWM77CCK1QF&limit=500"
            );
            if (!response.ok) return;
            const data = await response.json();
            const GIF = Object.values(data.results)[randomNo(0, data.results.length)];
            return send
              ? GIF.itemurl
              : {
                  image: {
